...
`nexus-rel-retry-grid` is centered on reliability work in Solidity. The technical target is: Develop a Solidity command-oriented project for retry scenarios with round-trip fixtures, lossless normalization checks, and no network dependency.
...
The score is deterministic. The current threshold is 181. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 4.
