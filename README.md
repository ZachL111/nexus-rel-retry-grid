...
nexus-rel-retry-grid is a Solidity project for reliability. It focuses on this technical goal: Develop a Solidity command-oriented project for retry scenarios with round-trip fixtures, lossless normalization checks, and no network dependency.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 181, risk penalty 6, latency penalty 4, and weight bonus 4. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
